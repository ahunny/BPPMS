...
const Chat = props => {
  ...
  const SendChat = async newMessage => {
    ...
    const data = {
      ...
      receiver_id: 1, // Hardcoded receiver ID, change if needed
  ...
  const fetchChat = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/Chat/GetChats?groupid=1`);
    ...
    return (
      <Bubble
        ...
        wrapperStyle={{
          right: {
            backgroundColor:
              props.currentMessage.user._id === userid ? 'lightblue' : 'grey',
          },
        }}
      />
    );
