...
const SetMeetGrades = props => {
  const {MeetDetail} = props.route.params;
  console.log(MeetDetail);
  const {userid} = props.route.params;
  console.log(userid);
  var projectid = MeetDetail.project_id;
  var meetingid = MeetDetail.meeting_id;
  ...
  const fetchCriteria = async () => {
    try {
      const response = await fetch(
        `${API_URL}/Grading/GetGradeCriteria?meetingid=${meetingid}`,
      );
      const data = await response.json();
      const formattedData = data.map(item => ({
        key: item.score_id.toString(),
        value: item.score_criteria + '  (' + item.score_weight + ')',
      }));
